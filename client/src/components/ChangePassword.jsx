...
const ChangePassword = () => {
    ...
    const confirmPassword = async(e) => {
        e.preventDefault();
        if(password !== ''){
            const response = await axios.post('http://localhost:8080/verifiedClient/confirmPassword', {token: getCookie('connect.sid'), id: getCookie('client.sid'), password: password});
    ...
    const resetPassword = async(e) => {
        ...
        if(password === newPassword){
            ...
            if(!invalid && doesMatch){
                const response = await axios.post('http://localhost:8080/verifiedClient/changePassword', {token: getCookie('connect.sid'), id: getCookie('client.sid'), newPassword: newPassword});
