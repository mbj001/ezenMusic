...
function Album({music_id, details}) {
    ...
    useEffect(() => {
        Axios.get("http://localhost:8080/ezenmusic/detail/album/"+music_id)
        .then(({data}) => {
            setDetailMusic(data);
        })
        .catch((err) => {
        ...
        <>
        {
            detailMusic.map((item, index) => (
                <StyledDetail>
                    ...
                    <div className="mb-[40px]">
                        {
                            initNum === "albumtrack" || initNum === undefined?
                                <div>
                                    <Link to={"/detail/album/" + music_id + "/intro"} className="rounded-[20px] px-[15px] py-[7px] mr-[10px] text-gray-600 font-normal" onClick={(e) => setInitNum("intro")}>상세정보</Link>
                                    <Link to={"/detail/album/" + music_id + "/albumtrack"} className="active rounded-[20px] px-[15px] py-[7px] mr-[10px] text-gray-600 font-normal" onClick={(e) => setInitNum("albumtrack")}>수록곡</Link>
                                </div>
                                :
                                <div>
                                    <Link to={"/detail/album/" + music_id + "/intro"} className="active rounded-[20px] px-[15px] py-[7px] mr-[10px] text-gray-600 font-normal" onClick={(e) => setInitNum("intro")}>상세정보</Link>
                                    <Link to={"/detail/album/" + music_id + "/albumtrack"} className="rounded-[20px] px-[15px] py-[7px] mr-[10px] text-gray-600 font-normal" onClick={(e) => setInitNum("albumtrack")}>수록곡</Link>
                                </div>
                    ...
                    {
                        initNum === "albumtrack"?
                            <AlbumTrack id={music_id} album_title={item.album_title} />
                            :
