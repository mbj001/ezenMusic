    ...
    const delPlaylist = () => {
        ...
        for(let i=0; i<likeAlbumList.length; i++){
            if(likeAlbumList[i].delcheckVal === true){
                array.push(likeAlbumList[i].artist_num);
            }
        }
        console.log(array);
                ...
                }
                {
                    likeAlbumList.map((item, index) => (
                        <div key={index} className="relative album-box col-4 flex items-center mb-[40px]">
                            ...
                            <Link to={"/detail/artist/"+item.artist_num+"/artisttrack"}><img src={"/image/artist/"+item.org_artist_img} alt="cover_image" className={ item.delcheckVal? "w-[175px] h-[175px] rounded-[50%] brightness-75" : "w-[175px] h-[175px] rounded-[50%]"} /></Link>
